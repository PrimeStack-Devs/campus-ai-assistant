...
float lineFn(vec2 st, float width, float perc, float offset, vec2 mouse, float time, float amplitude, float distance) {
    ...
    float time_scaled = time / 11.0 + (mouse.x - 0.5) * 0.8;
    float blur = 0.2 + 0.8 * perc;
...
export interface ThreadsProps extends React.HTMLAttributes<HTMLDivElement> {
...
export function Threads({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('WebGL initialization failed for Threads background', err);
      return;
    }
    ...
    function resize() {
      ...
      program.uniforms.iResolution.value.b = gl.canvas.width / (gl.canvas.height || 1);
    }
